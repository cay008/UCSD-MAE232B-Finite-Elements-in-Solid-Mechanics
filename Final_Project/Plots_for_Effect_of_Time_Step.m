...
%     for i = 2 : 3
%         t = 0:dt_all(1,i):5;
%         plot(t, sol_disp{1,i}(index,:), plot_string{i},'LineWidth',2);
%     end
%     legend('$\Delta t = 0.98\Delta t_{crit}$','$\Delta t = 0.5\Delta t_{crit}$','interpreter','latex','Location','southeast')
% end
% 
% % Plots for central difference
% for loc = 1 : 3
...
%     for i = 1 : 3
%         t = 0:dt_aa(i):5;
%         plot(t, sol_disp{2,i}(index,:),plot_string{i},'LineWidth',2);
%     end
%     legend('$\Delta t = 0.01$','$\Delta t = 0.001$','$\Delta t = 0.0005$','interpreter','latex','Location','southeast')
% end

%% Displacement Plot
% Plots for central difference
string = {'$\sigma_{rr}|_{r=13}$','$\sigma_{rr}|_{r=15}$','$\sigma_{rr}|_{r=17}$'};
plot_string = {'k-.','r-','b--'};
for loc = 1 : 3
    figure; hold on;
    xlabel('t'); ylabel(string{loc},'interpreter','latex');
    ...
    for i = 2 : 3
        t = 0:dt_all(1,i):5;
        plot(t, sol_sigma_rr{1,i}(1,:), plot_string{i},'LineWidth',2);
    end
    legend('$\Delta t = 0.98\Delta t_{crit}$','$\Delta t = 0.5\Delta t_{crit}$','interpreter','latex','Location','southwest')
end
...
for loc = 1 : 3
    ...
    for i = 1 : 3
        t = 0:dt_aa(i):5;
        plot(t, sol_sigma_rr{2,i}(1,:),plot_string{i},'LineWidth',2);
    end
    legend('$\Delta t = 0.01$','$\Delta t = 0.001$','$\Delta t = 0.0005$','interpreter','latex','Location','southwest')
end